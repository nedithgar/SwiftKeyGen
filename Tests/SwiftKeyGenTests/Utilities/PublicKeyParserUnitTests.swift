...
struct PublicKeyParserUnitTests {    
    ...
    @Test func validateKeyDataRSAOnly() throws {
        ...
        let keyData = key.publicKeyData()
        try PublicKeyParser.validatePublicKeyData(keyData, type: .rsa)
    }
    ...
    @Test func fingerprintFromString() throws {
        ...
        let parsedFingerprint = try PublicKeyParser.fingerprint(from: publicKeyString)
    ...
    @Test func multipleHashTypesNonRSA() throws {
        ...
        for keyType in keyTypes {
            ...
            let sha256 = try PublicKeyParser.fingerprint(from: publicKeyString, hash: .sha256)
            let sha512 = try PublicKeyParser.fingerprint(from: publicKeyString, hash: .sha512)
            let md5 = try PublicKeyParser.fingerprint(from: publicKeyString, hash: .md5)
