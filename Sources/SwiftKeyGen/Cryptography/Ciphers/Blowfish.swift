...
struct BlowfishContext {
    ...
    /// Encrypts two 32-bit words
    private mutating func encipher(_ xl: inout UInt32, _ xr: inout UInt32) {
        var Xl = xl
        var Xr = xr
    ...
    mutating func encrypt(data: inout [UInt32], blocks: Int) {
        for i in 0..<blocks {
            var xl = data[i * 2]
            var xr = data[i * 2 + 1]
            encipher(&xl, &xr)
